...
> Note: this reference includes historical examples from the broader Python SDK effort. For the currently shipped `src/` package surface and repo status, use the top-level `README.md` and `PROJECT_SPECS.md` as the source of truth.
...
export GOVERNS_BASE_URL="https://api.governs.ai"
...
client = GovernsAIClient(
    ...
    base_url=os.getenv("GOVERNS_BASE_URL", "https://api.governs.ai")
)
```
