...
class ContextClient:
    ...
    async def search_context_llm(self, input_data: Dict[str, Any]) -> ContextLLMResponse:
        """Search context using LLM-optimized compressed response format."""
        try:
            response = await self.http_client.post("/api/v1/context/search/llm", data=input_data)
    ...
    async def get_or_create_conversation(self, input_data: Dict[str, Any]) -> ConversationSummary:
        """Get or create a conversation for a given agent."""
        try:
            response = await self.http_client.post("/api/v1/context/conversation", data=input_data)
    ...
    async def get_conversation_context(
        ...
        try:
            ...
            contexts = response.data.get("contexts", []) if isinstance(response.data, dict) else []
            return [
    ...
    async def maybe_save_from_precheck(
        ...
        payload = precheck.to_dict() if isinstance(precheck, PrecheckResponse) else precheck
        ...
        if isinstance(save_action, dict) and isinstance(save_action.get("metadata"), dict):
    ...
    async def search_memory(self, params: Dict[str, Any]) -> MemorySearchResponse:
        ...
        try:
            response = await self.http_client.post("/api/v1/context/search", data=payload)
    ...
    async def resolve_user(self, params: Dict[str, Any]) -> ResolvedUser:
        ...
        try:
            response = await self.http_client.post("/api/v1/users/resolve", data=payload)
    ...
    async def get_user_by_external_id(
        ...
        query = urlencode({"externalUserId": external_user_id, "externalSource": external_source})
    ...
    def _to_save_context_payload(self, input_data: Union[SaveContextInput, Dict[str, Any]]) -> Dict[str, Any]:
