...
class DocumentClient:
    ...
    async def upload_document(
        ...
        try:
            response = await self.http_client.post_form_data("/api/v1/documents", form_data)
    ...
    async def get_document(
        ...
        try:
            response = await self.http_client.get(f"/api/v1/documents/{document_id}", params=params)
    ...
    async def search_documents(self, params: Dict[str, Any]) -> DocumentSearchResponse:
        ...
        try:
            response = await self.http_client.post("/api/v1/documents/search", data=params)
    ...
    def _normalize_file(
        ...
        raise GovernsAIError(
            "Unsupported file type. Use bytes, bytearray, memoryview, file path, or file-like object."
        )
