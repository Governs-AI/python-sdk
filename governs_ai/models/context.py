...
class ContextLLMResponse:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ContextLLMResponse":
        return cls(
            ...
            high_confidence=int(data.get("highConfidence", data.get("high_confidence", 0))),
            medium_confidence=int(data.get("mediumConfidence", data.get("medium_confidence", 0))),
            low_confidence=int(data.get("lowConfidence", data.get("low_confidence", 0))),
...
class MemorySearchResponse:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "MemorySearchResponse":
        ...
        return cls(
            success=bool(data.get("success", False)),
            memories=[MemoryRecord.from_dict(item) for item in raw_memories if isinstance(item, dict)],
            count=int(data.get("count", len(raw_memories))),
            metadata=MemorySearchMetadata.from_dict(raw_metadata)
...
class ResolvedUser:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "ResolvedUser":
        ...
        return cls(
            internal_user_id=data.get("internalUserId", data.get("internal_user_id", "")),
